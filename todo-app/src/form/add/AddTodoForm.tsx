...
function AddTodoForm() {
	const { state: addTodoState, runAsyncProcess: runAddTodoAsyncProcess } =
	...
	async function handleSubmit(event: FormEvent<HTMLFormElement>) {
		...
		try {
			const response = await runAddTodoAsyncProcess(
				todoApi.addTodo({ content: todoQuery, isCompleted: false })
			)
