...
function TodoListItem({ todo }: TodoListItemProps) {
    ...
    const { state: deleteTodoState, runAsyncProcess: runDeleteTodoAsyncProcess } = useAsyncProcess();
    ...
    return (
        <li>
            ...
            <div>
                ...
                <Button shouldDisplaySpinner={deleteTodoState.isRequestPending} onClick={handleDeleteClick}>
                    -
                </Button>
            </div>
        ...
        return async () => {
            try {
                ...
                await runUpdateTodoAsyncProcess(
                    todoApi.updateTodo({ ...todo, isCompleted: !todo.isCompleted }, todo.id),
                );
            } catch (error) {
                console.log(error);
            }
