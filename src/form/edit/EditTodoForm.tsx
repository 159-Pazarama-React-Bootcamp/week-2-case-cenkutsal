...
function EditTodoForm({ todo, onSubmit }: EditTodoFormProps) {
    const { state: editTodoState, runAsyncProcess: runEditTodoAsyncProcess } = useAsyncProcess<Todo>();
    ...
    async function handleSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            const response = await runEditTodoAsyncProcess(
                todoApi.updateTodo({ content: todoQuery, isCompleted: todo.isCompleted }, todo.id),
            );
    ...
    function handleTextFieldChange({ currentTarget: { value } }: React.SyntheticEvent<HTMLInputElement>) {
        setTodoQuery(value);
    }
}
